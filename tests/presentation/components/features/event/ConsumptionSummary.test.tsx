...
describe('ConsumptionSummary', () => {
  it('renders the empty message for a user with no participation', async () => {
      ...
      purchases: [
        {
          ...
          dailyConsumption: 1,
          totalQuantity: 6,
          consumers: [{ userId: 'u2', multiplier: 1 }],
          deleted: false,
          deletedBy: null,
          deletedAt: null,
          deleteReason: null,
    ...
    render(
      <Wrap event={event}>
        <ConsumptionSummary userId="u1" />
      </Wrap>,
    )
    expect(
      await screen.findByText(
        /disfruta del evento|enjoy the event|gaudeix|gaudix|ondo pasa|disfruta do evento/i,
      ),
    ).toBeInTheDocument()
  })
  ...
  it('renders headline + detail + closing for a user with consumption', async () => {
    ...
    expect(
      screen.getByText(
        /recortar la lista|trim the list|retallar la llista|retallar a lista|murriztu/i,
      ),
    ).toBeInTheDocument()
  })
