...
async function setup() {
  ...
  await new DeletePurchaseHandler(repo).execute({
    eventId: create.event.id, purchaseId, deletedBy: create.creator.id,
  })
...
describe('RecoverPurchaseHandler', () => {
  it('recovers a deleted purchase and records history', async () => {
    ...
    const result = await new RecoverPurchaseHandler(ctx.repo).execute({
      eventId: ctx.eventId, purchaseId: ctx.purchaseId, recoveredBy: ctx.userId,
    })
  ...
  it('rejects unknown purchase', async () => {
    ...
    await expect(
      new RecoverPurchaseHandler(ctx.repo).execute({
        eventId: ctx.eventId, purchaseId: '00000000-0000-7000-8000-000000000000', recoveredBy: ctx.userId,
      }),
  ...
  it('rejects recoveredBy not in event', async () => {
    ...
    await expect(
      new RecoverPurchaseHandler(ctx.repo).execute({
        eventId: ctx.eventId, purchaseId: ctx.purchaseId, recoveredBy: '00000000-0000-7000-8000-000000000000',
      }),
