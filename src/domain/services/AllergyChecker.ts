...
const KEYWORDS: Record<AllergenName, string[]> = {
  gluten: ['gluten', 'bread', 'pasta', 'flour', 'biscuit', 'cereal', 'wheat', 'pan', 'harina', 'galleta', 'trigo', 'cebada'],
  lactose: ['lactose', 'lactosa', 'milk', 'cheese', 'butter', 'yogurt', 'ice cream', 'leche', 'queso', 'mantequilla', 'yogur', 'helado', 'nata'],
  egg: ['egg', 'eggs', 'mayonnaise', 'huevo', 'huevos', 'mayonesa', 'tortilla'],
  peanut: ['peanut', 'peanuts', 'cacahuete', 'cacahuetes', 'maní'],
  nuts: ['nut', 'nuts', 'almond', 'hazelnut', 'walnut', 'cashew', 'nuez', 'nueces', 'almendra', 'avellana', 'anacardo', 'frutos secos', 'fruto seco'],
  shellfish: ['shellfish', 'shrimp', 'prawn', 'lobster', 'crab', 'gamba', 'gambas', 'camarón', 'langosta', 'cangrejo', 'marisco', 'mariscos'],
  ...
  mollusks: ['mollusk', 'mollusks', 'mollusc', 'molluscs', 'molusco', 'moluscos', 'mussel', 'oyster', 'clam', 'squid', 'octopus', 'mejillón', 'ostra', 'almeja', 'calamar', 'pulpo'],
  other: [],
}
  ...
  findMatches(input: AllergyCheckInput): AllergyMatch[] {
    ...
    for (const user of input.users) {
      for (const a of user.allergies) {
        ...
        const kws =
          a.name === 'other' ? (a.notes ? [a.notes.trim()] : []) : KEYWORDS[a.name]
